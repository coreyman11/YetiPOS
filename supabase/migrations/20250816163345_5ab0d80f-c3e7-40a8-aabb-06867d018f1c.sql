-- Phase 1: Database Foundation for Vendor Admin & Feature Toggle System (Fixed)
...
CREATE TABLE public.vendor_admin_audit_log (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  admin_user_id UUID REFERENCES auth.users(id), -- Made nullable for system operations
...
-- Create trigger to log feature toggle changes (only when user is authenticated)
